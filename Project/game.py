...
class Game:
    ...
    def roll(self):
        ...
        self.highlight_active_players(color)
    ...
    def highlight_active_players(self, color):
        for token_name, token_id in self.players[color].tokens.items():
            if token_name.startswith(color):
                self.canvas.itemconfig(token_id, fill="gold", outline="gold", width=3)
            else:
                self.canvas.itemconfig(token_id, outline="black", width=3)
        self.players[color].show_active_player(1)
